/*
 From randomForestB. This is a mix of the basic first code that just split on midrange. It still splits that way,
 but now goes through all of the possible splits into L and R nodes, stores information and computes the median for
 each node. The creates a vector of deviations from the appropriate median and performs a split by minimizing the median of the absolute deviations.
 This code works. 
*/
...
void findBestSplit(double *x, int *jdex, double *y, int mdim, int nsample,
  ...
  double medL, medR, medD, sumdev;
  double *righty;
  double *ynode;
  double *deviations;
  deviations=(double *) Calloc(nsample, double);
  righty = (double *) Calloc(nsample, double);
  ynode = (double *) Calloc(nsample, double);
  ...
  *decsplit = 0.0;
  ...
  for (i = 0; i < mtry; ++i) {
    ...
    rind=0;
    for (j = ndstart; j <= ndend; ++j) {
      ynode[j]=yl[ncase[j] - 1];
      }
    /* Search through the "gaps" in the x-variable. */
    for (j = ndstart; j <= ndend - 1; ++j) {
      npopl++;
      npopr--;
      /* Can sort Left indices directly since we won't need to worry about their ordering for future splits */
      R_qsort(ynode, ndstart + 1, j + 1);   /* indices are +1 since using R */
      medL=( npopl % 2 == 0)? (ynode[ndstart+npopl/2-1]+ynode[ndstart+(npopl)/2])/2: (ynode[(npopl-1)/2+ndstart]);
      /* Need a new vector right node, since we need to maintain order wrt x for future splits */
      for(rind=j+1; rind<=ndend; ++rind){
        righty[rind-(j+1)]=ynode[rind];
      }
      R_qsort(righty, 1, npopr);  /* indices are +1 since using R */
      medR=( npopr % 2 == 0)? (righty[npopr/2-1]+righty[(npopr)/2])/2: (righty[(npopr-1)/2]);
      d = yl[ncase[j] - 1];
      sumdev=0;
      for(dind=ndstart; dind<=j; ++dind){
        sumdev +=fabs(ynode[dind]-medL);
      }
      for(dind=j+1; dind<=ndend; ++dind){
        sumdev +=fabs(righty[dind-(j+1)]-medR);
      }
